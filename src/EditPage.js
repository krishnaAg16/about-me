...
export default function Editor({ obj, setObj }) {
    ...
    const postPage = async () => {
        try {
            const response = await axios.post(`${process.env.BASE_URL}:${process.env.API_PORT}/api/data`, obj);
    ...
    return (
        ...
        <div className="flex bg-gradient-to-tr from-red-600 via-cyan-800 via-45% to-violet-500 h-screen">

            {!publish &&
                <>
                    <div className="left flex flex-col w-1/2 p-5">
                    ...
                    <div className='bg-[#ececef] h-[98%] w-[49%] rounded overflow-y-auto my-auto'>
            ...
            {publish &&
                confetti_func() &&
                <div className="bg-[#1b1c25] w-full z-30 flex flex-col items-center text-white">
                    <span className="text-4xl mt-36 text-center px-2 py-3"> HURAAYYY!!! </span>
                    <span className="text-5xl mt-25 text-center px-2 py-3"> Portfolio Published</span>
                    <div className="text-2xl mt-32">Visit your Portfolio <Link to={obj.slug} replace={true} className="text-yellow-400 hover:text-yellow-500 hover:underline">{obj.slug.toLowerCase()}</Link></div>
                    <div className="relative mt-20 w-2/5">
                        <input type="text" ref={inputRef} value={publish} disabled className="h-12 w-full rounded-lg border border-stroke bg-transparent py-3 pl-5 pr-14 text-dark outline-none duration-200 selection:bg-transparent focus:border-primary " />
                        <button onClick={copyToClipboard} className="absolute right-0 top-0 flex h-12 w-14 items-center justify-center text-body-color duration-200 hover:text-primary">
                            {copySuccess ? (
                                <svg
                                    width="20"
                                    height="20"
                        ...
                        <div className="mt-5">Note: This ID will be essential for making updates and adjustments once the editing feature is available.
                            &nbsp; Please keep this ID confidential and do not share it with anyone to ensure the security of your portfolio.</div>
                    </div>
                </div>
            }
        </div >
